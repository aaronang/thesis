...
First, the diagnostic performance of SBFL techniques relies on the quality of the test suite.
SBFL techniques cannot accurately pinpoint a fault with a single failing test that covers multiple components, e.g.\ methods.
...
Second, in one of the pioneering user studies performed by Parnin and Orso~\cite{Parnin:2011:ADT:2001420.2001445}, the authors show that several assumptions made by SBFL techniques do not hold in practice.
...
However, these metrics do not enforce a good test suite.
For example, one could easily obtain a 100\% test coverage with a set of tests that each covers a big part of the codebase.
In this case, when a test fails due to a bug, it might be difficult for the developer to pinpoint the root cause.

To address the first problem --- SBFL techniques rely on the quality of the test suite --- Perez \etal~\cite{DBLP:conf/icse/PerezAD17} propose a new metric, called DDU, to quantify the diagnosability of a test suite.
As opposed to optimizing a test suite with respect to adequacy measurements that focus on error detection, e.g.\ branch coverage, line coverage.
...
Diagnosability is the property of faults to be easily and precisely locate~\cite{730889}.
...
For example, \texttt{com.google.inject.internal.ProvidesMethodScanner} has 35 branches and its partial activity matrix is shown in \Cref{tab:providesmethodscanner}.
...
\scriptsize
\centering
\caption{Activity matrix of the \texttt{org.apache.commons.math4.fitting.leastsquares.CircleProblem} class. Every transaction hits every component.}%
...
In the partial activity matrix of \texttt{Provides\-Method\-Scanner}, see \Cref{tab:providesmethodscanner}, the columns represent branches of \texttt{Provides\-Method\-Scanner}.
...
In fact, \texttt{Provides\-Method\-Scanner} has 35 components and if a transaction only hits a few components, it results in the activity matrix to become sparse.
...
The normalized density is low: 0.0, because the density is high: 1.0.
...
In Commons Math, the \texttt{Elitistic\-List\-Population} class has a normalized density of 0.979.
In its activity matrix, see \Cref{tab:elitistic}, we observe that there is a good balance between tests that cover a few components and tests that cover many components.
This results in a density close to 0.5 and a nearly optimal normalized density of 1.0.
...
The second reason is that for some classes there exist only one test, and in the current Python script the diversity defaults to 0 when there is only one test.
...
Although parameterized testing is a common practice to test different inputs for a unit, it has a negative effect on the diversity due to identical activity patterns.
...
We observe that 176 classes have a DDU of zero due to either normalized density, diversity, or uniqueness being equal to zero.
...
For example, the diversity can be negatively impacted when we write parameterized tests due to inputs causing the same activation patterns.
...
In other words, the DDU can only be as high as the component with the lowest value.
...
In \Cref{subfig:low_uniqueness} and \Cref{subfig:low_uniqueness_high_coverage}, we observe that branch coverage can be high or low given an activity matrix with a low uniqueness.
If the uniqueness is high, then the number of unique columns is high and therefore the branch coverage has to be high too.
In \Cref{subfig:optimal_uniqueness_coverage} and \Cref{subfig:optimal_uniqueness}, we observe two activity matrices with optimal uniqueness, resulting in a relative high or optimal coverage.
...
In \Cref{fig:error_detection_metric}, we observe that there is no strong correlation between normalized density, diversity, uniqueness, DDU, and error detection.
In \Cref{fig:delta_error_detection_metric}, we observe similar results; no strong correlation between delta normalized density, delta diversity, delta uniqueness, delta DDU, and delta error detection, i.e.\ improving the components of DDU does not result in an improvement in the error detection.
